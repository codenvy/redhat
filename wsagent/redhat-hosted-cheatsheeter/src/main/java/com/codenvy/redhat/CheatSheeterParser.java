...
public class CheatSheeterParser {
  ...
  private static final String RESOURCE_PATH =
      "https://raw.githubusercontent.com/codenvy-templates/redhat-kitchensink-webapp/master/.cheatsheet.xml";
  ...
  public String parse() throws ServerException {
    ...
    try {
      String[] commands =
          new String[] {
            binaryPath.toAbsolutePath().toString(), "-file=" + RESOURCE_PATH, "-out=" + HTML_NAME
          };
      ...
      if (exitCode != 0) {
        throw new ServerException(
            format(
                "Failed to launch cheatsheets docs parser. Parser binary exit code: '%s'",
                exitCode));
      }
      result = com.google.common.io.Files.toString(outPutFile, UTF_8);
    } catch (InterruptedException | IOException e) {
      throw new ServerException(
          format("Failed to parse cheatseets docs. Cause: '%s'", e.getMessage()));
    }
  ...
  private void prepareParser() {
    ...
    try {
      ...
      InputStream parserInStream =
          Thread.currentThread().getContextClassLoader().getResourceAsStream(PARSER_BINARY_NAME);
